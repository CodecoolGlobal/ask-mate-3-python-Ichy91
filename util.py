...
def get_questions_in_right_order(column_name, order_direction):
    return (data_handler.order_list_descending(column_name) if column_name and order_direction == 'DESC'
            else data_handler.get_datas_order_by_ASC('question', column_name) if column_name and order_direction == 'ASC'
            else data_handler.get_datas_order_by_ASC('question', 'submission_time'))
...
def get_question_id_by_comments(comments, comment_id):
    return next(comment['question_id'] for comment in comments if comment['id'] == comment_id)
